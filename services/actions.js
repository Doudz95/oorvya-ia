...
const { getPlaceType, getSectorVocab } = require('../utils/sector');
...
function detectIntent(message) {
  ...
  if ((m.includes('publie') || m.includes('publier') || m.includes('poste') || m.includes('ecris') || m.includes('annonce')) &&
      (m.includes('actualite') || m.includes('annonce') || m.includes('information') || m.includes('news') || m.includes('equipe')))
    return 'news_publish';
  ...
  const confirmWords = ['oui', 'ok', 'confirme', 'je confirme', 'valide', 'vas y', 'vasi', 'go', 'd accord'];
  if (confirmWords.includes(m)) return 'confirmation';

  return 'question';
}

// ─── EXTRACTION ACTION EN ATTENTE ─────────────────────────────────────────────
...
function parseNewsAction(message) {
  ...
  const titleMatch = original.match(/(?:titre[:\s]+|annonce[:\s]+)([^"'\n]+?)(?:\s+message|\s+contenu|[:\n]|$)/i) || quoted;
  const msgMatch = original.match(/(?:message[:\s]+|contenu[:\s]+|texte[:\s]+|dit[:\s]+)(.+)$/is);
  let title = titleMatch ? titleMatch[1].trim() : '';
  let msgContent = msgMatch ? msgMatch[1].trim() : '';
  if (!title && !msgContent) {
    const clean = original.replace(/^(?:publie|annonce|actualite|informe|poste)\s+/i, '');
...
module.exports = { detectIntent, extractPendingAction, buildConfirmationMessage, parseSiteAction, parseNewsAction, parseEmployeeUpdate, executeSiteCreate, executeNewsPublish };
